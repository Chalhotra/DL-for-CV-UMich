...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = hidden * (hidden > 0)   # ReLU without in-place ops
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    norm_scores = scores - scores.max(dim=1, keepdim=True).values.reshape(-1,1)
    softmax_scores= norm_scores.softmax(dim=1)
    correct_scores= torch.zeros_like(softmax_scores)
    correct_scores[torch.arange(X.shape[0]), y]=1
    # loss = -1*torch.log(softmax_scores[torch.arange(X.shape[0]), y]).sum()
    # loss /= X.shape[0]
    log_probs = norm_scores - (softmax_scores.exp().sum(dim=0)).log()
    probs = log_probs.exp()
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dscores = softmax_scores.clone()
    dscores[torch.arange(X.shape[0]), y]-=1
    dscores /= N



    dW2 = h1.t().matmul(dscores)
    db2 = dscores.sum(dim=0)
    dW2 += 2*reg*(W2)


    mask = (X.matmul(W1)+b1 >0).float()
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    hidden_lin= X.matmul(params['W1'])+params['b1']
    mask = (hidden_lin>0).float()
    h1 = hidden_lin*mask
    scores = h1.matmul(params['W2'])+params['b2']
    norm_scores = scores - scores.max(dim=1).values.reshape(-1,1)
    softmax_scores= norm_scores.softmax(dim=1)
    y_pred = softmax_scores.argmax(dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    # learning_rate_decays = [0.99, 0.95]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
      ...
      for hs in hidden_sizes:
        for rs in regularization_strengths:
          for lrd in learning_rate_decays:
            nn = TwoLayerNet(data_dict['X_train'].shape[1], hs, data_dict['y_train'].max().item()+1)
            stat = nn.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], lr, lrd, rs, num_iters=3000, batch_size=1000)
            if max(stat['val_acc_history']) > best_val_acc:
              best_val_acc = max(stat['val_acc_history'])
              best_net = nn
              best_stat = stat
