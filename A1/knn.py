...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        tr_ex = x_train[i]
        ts_ex = x_test[j]
        dists[i][j]= ((tr_ex-ts_ex)**2).reshape(-1, 1).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dist = []
    for i in range(num_train):
     loc_dist= x_test-x_train[i]
     dist.append((loc_dist**2).flatten(1).sum(dim=1))
    dists = torch.stack(dist)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    test_flat = x_test.flatten(1)

    a1 = (train_flat**2).sum(dim=1, keepdim=True)
    a2 = (test_flat**2).sum(dim=1, keepdim=True).t()
    a3 = train_flat @ test_flat.t()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      a = dists[:,i]
      b = list(y_train[torch.topk(a, k, largest=False).indices])
      c = {x: b.count(x) for x in b}
      maxfreq= max(list(c.values()))
      d = [x for x,y in c.items() if y == maxfreq]
...
def knn_cross_validate(
    ...
    for i in k_choices:
      ...
      for idx in range(num_folds):
        ...
        valid_data = (x_train_folds)[idx]
        valid_labels= y_train_folds[idx]
        
          # get train folds as lists
        train_parts = x_train_folds[:idx] + x_train_folds[idx+1:]
        train_label_parts = y_train_folds[:idx] + y_train_folds[idx+1:]

        if len(train_parts) == 0:
            raise ValueError("No training folds left after removing validation fold")

      # cat along sample dimension (usually dim=0). make sure shapes match.
        train_data = torch.cat(train_parts, dim=0)
        train_labels = torch.cat(train_label_parts, dim=0)
        kc = KnnClassifier(train_data, train_labels)
        k_to_accuracies[i].append(kc.check_accuracy(valid_data, valid_labels, k=i))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    a = max([sum(v)/len(v) for k,v in k_to_accuracies.items()])
    best_k = min([k for k,v in k_to_accuracies.items() if sum(v)/len(v) == a])
