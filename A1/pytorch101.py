...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    a = x[:12].reshape(3, 4)   # first block
    b = x[12:].reshape(3, 4)   # second block

    # Concatenate horizontally (dim=1)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y=x.copy_()
    z = y.min(dim=1)
    y[torch.arange(y.shape[0]), z.indices] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean_mat= x.sum(dim=0)/x.shape[1]
    stdev_mat = torch.sqrt(((x-mean_mat)**2).sum(dim=0)/(x.shape[1]-1))

    y = (x-mean_mat)/stdev_mat
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
